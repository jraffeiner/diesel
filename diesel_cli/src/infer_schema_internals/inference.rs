...
fn determine_column_type(
    ...
    #[expect(unused_variables)] primary_keys: Option<&[String]>,
...
fn load_view_sql_definition(
    ...
    match connection {
        ...
        #[cfg(feature = "mssql")]
        InferConnection::Mssql(mssql_connection) => Ok(
            super::information_schema::load_view_sql_definition(mssql_connection, name)?,
        ),
    }
}
