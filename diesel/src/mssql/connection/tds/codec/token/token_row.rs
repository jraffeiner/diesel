...
mod into_row;
use crate::mssql::connection::tds::codec::encode::Encode;
use crate::mssql::connection::{
    tds::codec::ColumnData, BytesMutWithTypeInfo, SqlReadBytes, TokenType,
};
use bytes::BufMut;
...
impl<'a> Encode<BytesMutWithDataColumns<'a>> for TokenRow<'a> {
    fn encode(self, dst: &mut BytesMutWithDataColumns<'a>) -> crate::mssql::connection::Result<()> {
...
impl<'a> TokenRow<'a> {
    ...
    pub(crate) fn iter(&self) -> std::slice::Iter<'_, ColumnData<'a>> {
        self.data.iter()
    }
    ...
    /// bounds.
    pub(crate) fn get(&self, index: usize) -> Option<&ColumnData<'a>> {
        self.data.get(index)
    }
...
impl TokenRow<'static> {
        ...
        R: SqlReadBytes,
    {
        ...
        for column in col_meta.columns.iter() {
            let data = ColumnData::decode(src, &column.base.ty)?;
            row.data.push(data);
        }
    ...
    {
        let col_meta = src.context().last_meta().unwrap();
        let row_bitmap = RowBitmap::decode(src, col_meta.columns.len())?;
        ...
        for (i, column) in col_meta.columns.iter().enumerate() {
            let data = if row_bitmap.is_null(i) {
                ...
                ColumnData::decode(src, &column.base.ty)?
            };
...
impl RowBitmap {
    ...
    /// type is `NbcRowToken`.
    fn decode<R>(src: &mut R, columns: usize) -> crate::mssql::connection::Result<Self>
    where
        R: SqlReadBytes,
    {
...
mod tests {
    use super::*;
    use crate::mssql::connection::{
        BaseMetaDataColumn, ColumnFlag, FixedLenType, MetaDataColumn, TypeInfo,
    };
    use bytes::BytesMut;

    fn wrong_number_of_columns_will_fail() {
