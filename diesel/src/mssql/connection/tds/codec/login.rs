...
#[derive(Debug, Clone, Default)]
#[cfg_attr(test, derive(PartialEq, Eq))]
pub(crate) struct LoginMessage<'a> {
...
impl<'a> LoginMessage<'a> {
    pub(crate) fn new() -> LoginMessage<'a> {
        Self {
            packet_size: 4096,
    ...
    #[cfg(windows)]
    pub(crate) fn integrated_security(&mut self, bytes: Option<Vec<u8>>) {
        if bytes.is_some() {
    ...
    pub(crate) fn app_name(&mut self, name: impl Into<Cow<'a, str>>) {
        self.app_name = name.into();
    }

    pub(crate) fn db_name(&mut self, db_name: impl Into<Cow<'a, str>>) {
        self.db_name = db_name.into();
    }

    pub(crate) fn server_name(&mut self, server_name: impl Into<Cow<'a, str>>) {
        self.server_name = server_name.into();
    }

    pub(crate) fn user_name(&mut self, user_name: impl Into<Cow<'a, str>>) {
        self.username = user_name.into();
    }

    pub(crate) fn password(&mut self, password: impl Into<Cow<'a, str>>) {
        self.password = password.into();
    }

    pub(crate) fn aad_token(
        &mut self,
    ...
    pub(crate) fn readonly(&mut self, readonly: bool) {
        if readonly {
...
mod tests {
    use super::*;
    use crate::mssql::connection::Decode;
