...
to_sql!(self_,
        ...
        /*chrono::DateTime<Tz: TimeZone>: (ColumnData::DateTimeOffset, {
            use chrono::Timelike;
    ...
    <Tz as TimeZone>::Offset: Send + Sync,
{
    fn to_sql(&self) -> ColumnData<'_> {
        ...
        let tz = self.fixed_offset().timezone();
        let offset = (tz.local_minus_utc() / 60) as i16;

        let res = DateTimeOffset::new(DateTime2::new(date, time), offset);
        ColumnData::DateTimeOffset(Some(res))
    }
}
